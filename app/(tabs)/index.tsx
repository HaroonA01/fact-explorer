...
import { DailyFactCard } from '../../src/components/DailyFactCard';
import { LoadingScreen } from '../../src/components/LoadingScreen';
import { SwipeUpHint } from '../../src/components/SwipeUpHint';
import { useTheme } from '../../src/contexts/ThemeContext';
import { Layout } from '../../src/constants/layout';
import { CATEGORIES } from '../../src/data/categories';
import { getFeaturedFactForDate } from '../../src/data/facts';
...
function Particle({
  ...
  const opa = useSharedValue(disabled ? 0.3 : 0);
...
export default function HomeScreen() {
  ...
  const featuredFact = getFeaturedFactForDate();
  ...
  return (
    <View style={[styles.root, { backgroundColor: rootBg }]}>
      ...
      <View style={StyleSheet.absoluteFill} pointerEvents="none" importantForAccessibility="no-hide-descendants">
        {PARTICLE_DATA.map((p) => (
          <Particle key={`bg-${p.id}`} {...p} color={particleColor} disabled={reduceMotion} />
        ))}
      </View>

      <Animated.View
        style={[styles.floatingTitle, floatingContainerStyle]}
        pointerEvents="none"
        accessibilityRole="header"
        accessibilityLabel="Fact Explorer"
      >
        <Animated.Text
      ...
      >
        ...
        <View style={styles.categoriesSection}>
          <Text style={[styles.sectionTitle, { color: heroTextColor }]}>Today</Text>
          <DailyFactCard fact={featuredFact} />

          <Text style={[styles.sectionTitle, { color: heroTextColor, marginTop: Layout.spacing.sm }]}>
            Categories
          </Text>
